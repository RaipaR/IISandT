...
public class Perceptron {
    ...
    public Perceptron(int inputSize, double learningRate) {
        this.weights = new double[inputSize + 1];  // +1 для смещения (bias)
        this.learningRate = learningRate;
        for (int i = 0; i < weights.length; i++) {
            weights[i] = Math.random() - 0.5;  // Значения в диапазоне [-0.5, 0.5]
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int[] labelsAnd = {0, 0, 0, 1};  // Результаты для "И"
        
        // Логическая функция "ИЛИ" (OR)
        int[][] inputsOr = {{0, 0}, {0, 1}, {1, 0}, {1, 1}};
        int[] labelsOr = {0, 1, 1, 1};  // Результаты для "ИЛИ"
        
        // Логическая функция "НЕ" (NOT)
        int[][] inputsNot = {{0}, {1}};
        int[] labelsNot = {1, 0};  // Результаты для "НЕ"
